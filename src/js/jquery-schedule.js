(function ($, window, document) {
  ...
  $.extend(Plugin.prototype, {
    ...
    init() {
      ...
      if (this.settings.periodColors.length > 0) {
        $.each(this.settings.periodColors, (index, color) => {
          if (!$.inArray(color) || color.length !== 4) {
      ...
      if (this.settings.mode === 'edit') {
        ...
        $(this.element).on('mousedown', '.jqs-day', (event) => {
          const offset = event.pageY - $(event.target).offset().top;
          ...
          if (!$(event.target).hasClass('jqs-period') && $(event.target).parents('.jqs-period').length === 0) {
            ...
            $(event.target).append(helper);
          }
        });

        $(this.element).on('mousemove', '.jqs-day', (event) => {
          if (helper) {
            const offset = event.pageY - $(event.target).offset().top;
        ...
        $(this.element).on('mouseup', '.jqs-day', (event) => {
          if (!$(event.target).hasClass('jqs-period') && $(event.target).parents('.jqs-period').length === 0) {
            const offset = event.pageY - $(event.target).offset().top;
            let height = Math.round(offset / $this.periodPosition) - position;
            if (height <= 0) {
            ...
            $this.add($(event.target), position, height);
          }
        ...
        $(this.element).on('mouseenter', '.jqs-day', function () {
          const index = $(this).parents('td').index();
        ...
        $(this.element).on('click', '.jqs-period-remove', function () {
          const period = $(this).parents('.jqs-period');
          $this.remove(period);
        });
        ...
        $(this.element).on('click', '.jqs-day-remove', function () {
          const index = $(this).parents('.jqs-grid-day').index();
     ...
     * @param options
     */
    add(parent, position, height, options) {
    ...
    openOptions(event, period) {
      ...
      '<input type="text" placeholder="'}${this.settings.periodTitlePlaceholder
          }" value="${title}" class="jqs-options-title form-control ${this.settings.classes.input}"></div>`;
      ...
      $('.jqs-options-title', this.element).on('keyup', (e) => {
        e.preventDefault();
        $('.jqs-period-title', period).text($(e.target).val());
        period.attr('title', $(e.target).val());
      });
